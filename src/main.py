...
def infer(model: Model, fn_img: Path, internal_call=False):
...
def infer_multiple(img_paths: List) -> Tuple:
    recg, probs = [], []
    preprocessor = Preprocessor(get_img_size(), dynamic_width=True, padding=16)
    fn_imgs = list(map(lambda x: cv2.imread(x, cv2.IMREAD_GRAYSCALE), img_paths))
    fn_imgs = list(map(preprocessor.process_img, fn_imgs))
    batch = Batch(fn_imgs, None, len(fn_imgs))
    for decoder_type in [DecoderType.BestPath]:
        model = Model(char_list_from_file(), decoder_type, must_restore=True)
        recognized, probabilities = model.infer_batch(batch, True)
        recg.append(recognized)
        probs.append(probabilities)

    return recognized, probabilities
